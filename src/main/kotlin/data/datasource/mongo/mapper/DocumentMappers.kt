...
fun User.toDocument(): Document {
    return Document(ID_FILED, this.id.toString())
        ...
        .append(ROLE_FILED, this.userRole.name)
...
fun Document.toUser(): User {
    return User(
        ...
        userRole = UserRole.valueOf(this.getString(ROLE_FILED)),
...
fun ProjectLog.toDocument(): Document =
    Document()
        .append(ID_FILED, id.toString())
        .append(USER_ID_FILED, userId.toString())
        .append(ENTITY_ID_FILED, entityId.toString())
        .append(PREVIOUS_ENTITY_FILED, previousEntity?.toDocument())
        .append(CURRENT_ENTITY_FILED, currentEntity?.toDocument())
        .append(CREATED_AT_FILED, createdAt.toString())

fun TaskLog.toDocument(): Document = Document()
    .append(ID_FILED, id.toString())
    .append(USER_ID_FILED, userId.toString())
    .append(ENTITY_ID_FILED, entityId.toString())
    .append(PREVIOUS_ENTITY_FILED, previousEntity?.toDocument())
...
fun Task.toDocument(): Document {
    return Document(ID_FILED, id.toString())
        .append(PROJECT_ID_FILED, projectId.toString())
        .append(TITLE_FILED, title)
        .append(DESCRIPTION_FILED, description)
        .append(STATE_FILED, taskState.toDocument())
...
fun Document.toState(): TaskState {
...
fun Project.toDocument(): Document {
    return Document(ID_FILED, id.toString())
        ...
        .append(CREATED_AT_FILED, createdAt.toString())
        .append(UPDATED_AT_FILED, updatedAt.toString())
        .append(STATE_FILED, taskStates.map {
            Document(ID_FILED, it.id.toString())
                .append(NAME_FILED, it.name)
        }).append(USERS_FILED, users.map {
            Document(ID_FILED, it.id.toString())
                .append(NAME_FILED, it.name)
                .append(PASSWORD_FILED, it.password)
                .append(EMAIL_FILED, it.email)
                .append(ROLE_FILED, it.userRole.name)
                .append(IS_DELETED_FILED, it.isDeleted)
        })
}

fun TaskState.toDocument(): Document {
    return Document()
        .append(ID_FILED, this.id.toString())
        .append(NAME_FILED, this.name)
}
    ...
    return (this[filedName] as? List<*>)?.mapNotNull { userDoc ->
        (userDoc as? Document)?.let {
            User(
                ...
                userRole = UserRole.valueOf(it.getString(ROLE_FILED)),
...
private fun Document.toStateList(): List<TaskState> {
