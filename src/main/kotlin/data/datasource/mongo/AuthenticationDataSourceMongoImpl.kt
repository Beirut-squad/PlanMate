...
class AuthenticationMongoDataSourceImpl(
    ...
    override suspend fun isValidEmail(email: String)
    : Boolean {
        return withContext(Dispatchers.IO) {
    ...
    override suspend fun registerMate(name: String, password: String, email: String)
    : User {
        return withContext(Dispatchers.IO) {
            if (!isEmailExists(email)) {
                val newUser = createUser(name, password, email, UserRole.MATE)
    ...
    override suspend fun registerAdmin(name: String, password: String, email: String)
    : User {
        return withContext(Dispatchers.IO) {
            if (!isEmailExists(email)) {
                val newAdmin = createUser(name, password, email, UserRole.ADMIN)
    ...
    private fun isEmailExists(email: String)
    :Boolean {
        return  mongoConnection
            .users
            .find(Document(EMAIL_FILED, email))
            .first() != null
    }
