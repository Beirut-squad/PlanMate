...
class GetProjectTasksUseCaseTest {
    ...
    fun `getProjectTasks should call repository with correct project ID`() = runTest {
        ...
        coEvery { taskRepository.getAllProjectTasks(any()) } returns taskList

        // When
        getProjectTasksUseCase.getProjectTasks(projectId)

        // Then
        coVerify(exactly = 1) { taskRepository.getAllProjectTasks(projectId) }
    }
    ...
    fun `getProjectTasks should return list of tasks from repository`() = runTest {
        ...
        coEvery { taskRepository.getAllProjectTasks(projectId) } returns expectedTasks
    ...
    fun `getProjectTasks should return specific tasks for the requested project ID`() = runTest {
        ...
        coEvery { taskRepository.getAllProjectTasks(projectId1) } returns tasksForProject1
        coEvery { taskRepository.getAllProjectTasks(projectId2) } returns tasksForProject2
    ...
    fun `getProjectTasks should ensure task list contains only tasks for requested project`() = runTest {
        ...
        coEvery { taskRepository.getAllProjectTasks(projectId) } returns tasks
