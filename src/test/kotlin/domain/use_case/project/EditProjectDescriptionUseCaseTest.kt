...
class EditProjectDescriptionUseCaseTest {
 ...
 fun `given valid new description, when editProject called, then should update project and log`() = runTest {
  ...
  // Then
  coVerify(exactly = 1) { projectRepository.editProject(withArg {
 ...
 @Test
 fun `given blank description, when editProject called, then should throw EmptyProjectDescriptionException`() = runTest {
