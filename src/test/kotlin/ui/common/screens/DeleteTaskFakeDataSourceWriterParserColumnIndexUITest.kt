...
class DeleteTaskFakeDataSourceWriterParserColumnIndexUITest {
    ...
    fun `should show message when no tasks are available`() {
        ...
        // Then
        verify { printer.printInfoLine("No tasks available to delete.") }
    ...
    fun `should show error when task retrieval fails`() {
        ...
        // Then
        verify { printer.printError("Failed to retrieve tasks: Failed to get tasks") }
        verify(exactly = 0) { deleteTaskUseCase.deleteTask(any(), any()) }
    }
    ...
    fun `should display tasks and handle invalid task selection`() {
        ...
        // Then
        verify { printer.printTitle("Select a Task to Delete:") }
        verify { printer.printInfoLine(match { it.contains("Task #1") }) }
        verify { printer.printLoader("Enter the task number to delete:") }
        verify { printer.printError("Invalid task number.") }
        verify(exactly = 0) { deleteTaskUseCase.deleteTask(any(), any()) }
    }
    ...
    fun `should confirm and delete task when user confirms with yes`() {
        ...
        // Then
        verify { printer.printInfoLine("Are you sure you want to delete the task: 'Test Task'? (yes/no)") }
        verify { deleteTaskUseCase.deleteTask(task, taskId) }
        verify { printer.printInfoLine("Task deleted successfully.") }
    }
    ...
    fun `should not delete task when user cancels with no`() {
        ...
        // Then
        verify { printer.printInfoLine("Deletion cancelled.") }
        verify(exactly = 0) { deleteTaskUseCase.deleteTask(any(), any()) }
    }
    ...
    fun `should handle error during task deletion`() {
        ...
        // Then
        verify { printer.printError("Failed to delete task: Deletion failed") }
    }
    ...
    fun `should correctly display multiple tasks`() {
        ...
        verifySequence {
            ...
            printer.printInfoLine(match { it.contains("Task #1") && it.contains("Test Task") })
            printer.printInfoLine(match { it.contains("Task #2") && it.contains("Another Task") })
            printer.printLoader("Enter the task number to delete:")
            printer.printInfoLine("Are you sure you want to delete the task: 'Another Task'? (yes/no)")
