...
class CreateNewTaskFakeDataSourceWriterParserColumnIndexUITest  {
    ...
    fun `should show error when project is not found`() {
        ...
        // Then
        verify { printer.printError("Failed to retrieve project: Project not found") }
    ...
    fun `should show error when current user is not available`() {
        ...
        // Then
        verify { printer.printError("No user found") }
        verify(exactly = 0) { createTaskUseCase.createTask(any(), any(), any(), any(), any()) }
    }
    ...
    fun `should prompt again if task name is blank and then accept valid input`() {
        ...
        verifySequence {
            ...
            printer.printInfoLine("Write your task name:")
            printer.printError("Input cannot be empty or null. Please try again.")
            printer.printInfoLine("Write your task name:")
            printer.printError("Input cannot be empty or null. Please try again.")
            printer.printInfoLine("Write your task name:")
            printer.printInfoLine("Tell me more about description of your task:")
            printer.printOptions("Choose a state for the task:")
            printer.printInfoLine("1. To Do")
        }
    ...
    fun `should show error when project retrieval fails`() {
        ...
        // Then
        verify { printer.printError("Failed to retrieve project: Error retrieving project") }
        verify(exactly = 0) { createTaskUseCase.createTask(any(), any(), any(), any(), any()) }
    }
    @Test
    fun `should keep prompting if task name is null or blank`() {
        ...
        // Then
        verify(exactly = 2) { printer.printError("Input cannot be empty or null. Please try again.") }
        verify {
    ...
    fun `should keep prompting until valid state is selected`() {
        ...
        // Then
        verify {
            printer.printError(match { it.contains("Invalid state selection") })
        }
        verify {
            createTaskUseCase.createTask("Task", "Desc", state, ProjectId, userId)
        }
    ...
    fun `should prompt again for task description if empty`() {
        ...
        // Then
        verify(exactly = 1) { printer.printError("Input cannot be empty or null. Please try again.") }
        verify {
            createTaskUseCase.createTask("Valid Task", "Valid Description", state, ProjectId, userId)
        }
    }
    ...
    fun `should handle multiple states in project`() {
        ...
        verify {
            printer.printInfoLine("1. To Do")
            printer.printInfoLine("2. In Progress")
            printer.printInfoLine("3. Done")
        }
