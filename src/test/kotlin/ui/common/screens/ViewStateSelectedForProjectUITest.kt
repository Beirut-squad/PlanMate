...
class ViewStateSelectedForProjectUITest{
  private val viewer: Viewer = mockk(relaxUnitFun = true)
...
fun tearDown(){
 ...
 fun `should navigate to ViewProjectsForUserUI when no states available for the project`() {
  ...
  every { viewer.readIntInput("Enter the number of the state to view (Enter Any Thing To Go Back): ") } returns 999
 ...
 fun `should print error when project retrieval failed`() {
  ...
  verify { viewer.printError("Failed to retrieve project: Something went wrong") }
 }
 ...
 fun `should print state options when states are available for the project`() {
  ...
  every { viewer.readIntInput("Enter the number of the state to view (Enter Any Thing To Go Back): ") } returns 1
 ...
 fun `should navigate to ViewProjectsForUserUI when invalid option is selected`() {
  ...
  every { viewer.readIntInput("Enter the number of the state to view (Enter Any Thing To Go Back): ") } returns null
 ...
 fun `should display tasks when tasks are retrieved successfully`() {
  ...
  every { viewer.readIntInput(any()) } returns 1
  ...
  verify { viewer.printInfoLine("Tasks:") }
  verify { viewer.printInfoLine(" - Name: Task 1, Description: Task description") }
  verify(exactly = 0) { viewer.printInfoLine("No tasks available for this state.") }
 }
 ...
 fun `should display 'No tasks available for this state' when no tasks for selected state`() {
  ...
  verify { viewer.printInfoLine("No tasks available for this state.") }
 }
 ...
 fun `should print error when tasks retrieval fails`() {
  ...
  verify { viewer.printError("Failed to retrieve tasks: Failed to retrieve tasks") }
 }
 ...
 fun `should display multiple states and navigate correctly based on user input`() {
  ...
  verify { viewer.printInfoLine("Tasks:") }
  verify { viewer.printInfoLine(" - Name: Task 1, Description: Description 1") }
 }
