...
class EditTaskFakeDataSourceWriterParserColumnIndexUITest {
    ...
    fun `should show message when no tasks are available`() {
        ...
        // Then
        verify { printer.printInfoLine("No tasks available to edit.") }
    ...
    fun `should display tasks and handle invalid task selection`() {
        ...
        // Then
        verify { printer.printTitle("Select a Task to Edit:") }
        verify { printer.printInfoLine(match { it.contains("Task #1") && it.contains("Test Task") }) }
        verify { printer.printLoader("Enter the task number to edit:") }
    ...
    fun `should show error when project retrieval fails`() {
        ...
        // Then
        verify { printer.printError("Failed to retrieve project: Failed to get project") }
        verify(exactly = 0) { editTaskUseCase.editTask(any(), any(), any(), any()) }
    }
    ...
    fun `should keep existing values when input is empty`() {
        ...
        // Then
        verify { printer.printTitle("Edit Task - Test Task") }
        verify { printer.printLoader(match { it.contains("Enter new Title") }) }
    ...
    fun `should handle exception during task update`() {
        ...
        // Then
        verify { printer.printError("Failed to update task: Update failed") }
    }
    ...
    fun `should show error when database is unavailable while retrieving tasks`() {
        ...
        // Then
        verify { printer.printError("Failed to retrieve tasks: Database not available") }
        verify(exactly = 0) { getProjectByIdUseCase.getProjectById(any()) }
        verify(exactly = 0) { editTaskUseCase.editTask(any(), any(), any(), any()) }
    }
