...
class ViewProjectForMateUITest{
 ...
 //Given
  ...
  verify {
   viewer.printError("Failed to retrieve project: Something went wrong")  }
 }
 ...
 fun `should print project info and goodbye when invalid option is selected`() {
  ...
  verify {
   viewer.printInfoLine(match { it.contains("Project Name: Test Project") })
   viewer.printOptions(any(), any(), any(), any())
